...
// this fun his job to see if the place the player want to move for 
// is valid place or not
// so if the player move to new block 
// we update the previous map into new with new plyer loc
...
// so when u move
// and remember each point are a full box 
// have a size of (1.0 x 1.0) this mean the
...
// the wall ZERO if it is it will make a problem 
// with rendering the map in the right way
// btw when u press W,S u r moving on the same line u see 
// so there is no need to do anything with the dirs just add or just minis
...
// and using X,Y so here another story 
// we move like 90deg from where i am looking
